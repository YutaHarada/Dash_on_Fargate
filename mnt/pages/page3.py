    ...
    [
        html.Div(
            [
                dcc.Graph(
                    id="output-map",
                    style={"width": "150vh", "height": "170vh"}
                )
            ]
        )
    ]
...
def update_graph(n_clicks, input_date, input_time):
    # 入力値に応じたDataFrameを抽出
    df_input_dt = \
        df.query("jst_date == @input_date and jst_time == @input_time")
    ...
    figure.update_layout(
        hovermode="closest",
        mapbox=dict(
            center=go.layout.mapbox.Center(lat=36.2488, lon=138.4768),
            pitch=0,
            zoom=5.5
        )
    )
