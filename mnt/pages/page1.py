...
sidebar = html.Div(
    [
        html.Div(
            [
                html.H2(["5days", html.Br(), "Information"]),
            ],
            style={
                "font-weight": 700,
                "margin-bottom": "20px",
    ...
    [
        html.Div(
            [
                dcc.Graph(
                    id="output-table",
                    style={"width": "150vh", "height": "155vh"}
                )
            ]
        )
    ]
...
# コールバック関数の設定
@callback(Output("output-table", "figure"), Input("dropdown_1", "value"))
    ...
    for i, date in enumerate(date_list, 1):
        ...
        table = go.Table(
            ...
            cells=dict(
                ...
                line_color="darkslategray",
                fill=dict(
                    color=["midnightblue"] + ["aliceblue"] * (n_columns - 1)
                ),
                align="center",
                font=dict(
                    color=["white"] + ["steelblue"] * (n_columns - 1), size=14
                ),
                height=30
            )
        )
